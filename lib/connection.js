...
class Connection extends EventEmitter {
  ...
  _resolveDefaultValues(options) {
    if(!options.values) return;
    if(!this.config.allowDefault) return;
    const command = options.sql.split(' ')[0];
    if(command.toLowerCase() != 'insert') return;

    const leftPart = options.sql.split('(')[0];
    let rightPart = options.sql.split(')')[1] + ')';
    let fields = options.sql.split('(')[1].split(')')[0].split(',');

    options.values = options.values.filter((value, index) => {
      if(value == 'DEFAULT') {
        let placeHolder = '?,';
        if(index == options.values.length - 1) placeHolder = '?'
        rightPart = rightPart.replace(placeHolder, '');
        fields = fields.filter((field, idx) => idx != index);
        return false;
      }
      else return true;
    })

    options.sql = leftPart + ' (' + fields.join(',') + ') ' + rightPart;
  }
  ...
  execute(sql, values, cb) {
    ...
    this._resolveDefaultValues(options)
