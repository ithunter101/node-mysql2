    ...
    switch (commandCode) {
      case CommandCode.STMT_PREPARE:
        if (connection.listeners('stmt_prepare').length) {
          const str = packet.readString(undefined, encoding);
          const query = str.split("::::")[0];
          const values = str.split("::::")[1];
          connection.emit('stmt_prepare', query, values);
        } else {
          connection.writeError({
      ...
      //     let nullBitmap; 
      //     if(packet.readNullBitmap) {
      //       nullBitmap = packet.readNullBitmap(connection.packetParser.fieldCount);
      //     }
           
      //     const values = [];
      //     for (let i = 0; i < connection.packetParser.fieldCount; i++) {
      //       if (nullBitmap && nullBitmap[i]) {
      //         values.push(null);
      //       } else {
      //         values.push(packet.readTypedValue(connection.packetParser.fields[i]));
      //       }
      //     }
